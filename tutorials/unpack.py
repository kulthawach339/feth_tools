    ...
    with open(file_path, "rb") as f:
        ...
        for language_idx in range(language_count):
            ...
            for _ in range(first_table.num_of_msg):
                first_table.ptrs.append(read_uint(f))
                skip(f, 16)
            for i in range(first_table.num_of_msg):
                f.seek(first_table.relative_pos + first_table.header_size + first_table.ptrs[i])
                strings.append(read_utf8_string_nt(f))
            f.seek(language_relative_offset + table_ptrs[1][0])
            strings.append("===")
            second_table = Table()
            ...
            for _ in range(second_table.num_of_msg):
                second_table.ptrs.append(read_uint(f))
            for i in range(second_table.num_of_msg):
                f.seek(second_table.relative_pos + second_table.header_size + second_table.ptrs[i])
                strings.append(read_utf8_string_nt(f))
            strings.append("======")
